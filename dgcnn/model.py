...
class DGCNN_cls_orig(nn.Module):
    def __init__(self, args, output_channels=40):
        super(DGCNN_cls_orig, self).__init__()
        self.args = args
        #self.k = args.k
        self.k = 20
...
class DGCNN_cls(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.relu(self.linear1(x)) # (batch_size, emb_dims*2) -> (batch_size, 512)
        x = F.relu(self.linear2(x)) # (batch_size, 512) -> (batch_size, 256)
        x = self.linear3(x)                                             # (batch_size, 256) -> (batch_size, output_channels)
        x = x.view(-1, 3, 3)
        return x
...
class DGCNN_cls3vec(nn.Module):
    def __init__(self, args, output_channels=40):
        super(DGCNN_cls3vec, self).__init__()
        self.args = args
        #self.k = args.k
        self.k = 20
        ...
        self.head1 = nn.Linear(output_channels, 3)
        self.head2 = nn.Linear(output_channels, 3)
        self.head3 = nn.Linear(output_channels, 3)
    ...
    def forward(self, x):
        ...
        x1, x2, x3 = self.head1(x), self.head2(x), self.head3(x)
