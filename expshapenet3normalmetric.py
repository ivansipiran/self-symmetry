...
comment = "Pointnet++ Lr 0.01, Calculate cosine distance on Shapenet airplane subset using 3 normals centered (0,0,0)," \
...
for net in nets:
    for lr in range(len(lrlist)):
        for i in range(len(lambdalist1)):
            os.system(f"python symmetry-metric-3normal.py --lr {lrlist[lr]} --name {mpath} --iterations 500 --export-interval 100 --pc "
                  f" {Path(modelpath) / Path(mpath)} --init-var 0.15 --D1 8000 --D2 8000 --sampling-mode curvature "
                  f"--save-path {Path(str(savepath)+'/'+mpath+'-'+f'{net}'+f'airplanes-cosinesimilarity-'+'-'+str(lrlist[lr])+'+'+str(lambdalist1[i])+'+'+str(lambdalist2[i])+'+'+str(lambdalist3[i]))} "
                  ...
                  f"--groundtruth_rotation_normals_path {rot_normals_path} "
